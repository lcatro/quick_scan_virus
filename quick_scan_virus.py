...
def search_hash_from_file(file_list) :
    ...
    for file_line in file_object :
        if file_line[0] is '#' or len(file_line) is 0 :
            continue
...
if len(sys.argv) is 2 :
    ...
    for file_scan_index in file_list :
        if file_scan_index.get('is_virus') is 'yes' :
